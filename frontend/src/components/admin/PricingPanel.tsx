...
export default function PricingPanel() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(STORAGE_KEYS.SUBSCRIPTION_PRICES)
      if (raw) { const p = JSON.parse(raw); setPrices(p); setDraft(p) }
    } catch { /* ignore */ }
  }, [])

  function savePrices(e: FormEvent<HTMLFormElement>) {
    e.preventDefault()
    setPrices(draft)
    localStorage.setItem(STORAGE_KEYS.SUBSCRIPTION_PRICES, JSON.stringify(draft))
  ...
  return (
    <section className="rounded-2xl border border-[#282828] bg-[#181818] p-5">
      <div className="mb-5">
        ...
        <p className="text-sm text-[#B3B3B3]">این بخش فقط برای مدیر سامانه فعال است.</p>
      </div>
      ...
      <form className="grid gap-4 md:grid-cols-[1fr_1fr_auto] md:items-end" onSubmit={savePrices}>
        <label className="block">
          ...
          <input
            className="input-field" min={0} type="number" value={draft.silver}
            onChange={e => updateDraft('silver', Number(e.target.value))}
          />
        </label>
        <label className="block">
          <span className="mb-2 block text-sm text-[#B3B3B3]">قیمت طلایی / تومان</span>
          <input
            className="input-field" min={0} type="number" value={draft.gold}
            onChange={e => updateDraft('gold', Number(e.target.value))}
          />
        </label>
